...
static const char *TAG = "X_SPEAKER";
...
void SpeakerApp::handleA2dpEvent(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
    ...
    case ESP_A2D_CONNECTION_STATE_EVT:
        ESP_LOGI(TAG, "A2DP connection state changed: %s, [%02x:%02x:%02x:%02x:%02x:%02x]",
        ...
        if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTING) {
            ...
            ESP_LOGI(TAG, "A2DP disconnected, set scan mode to connectable and discoverable, disable I2S channel");
            setScanModeConnectable(true, true);
            _audio_i2s.stop();
        }
        break;
    case ESP_A2D_AUDIO_STATE_EVT:
        ESP_LOGI(TAG, "A2DP audio state changed: %d", param->audio_stat.state);
        break;
    case ESP_A2D_AUDIO_CFG_EVT:
        ESP_LOGI(TAG, "A2DP audio config changed: codec type %d", param->audio_cfg.mcc.type);
    ...
    case ESP_A2D_PROF_STATE_EVT:
        ESP_LOGI(TAG, "A2DP profile state changed: %d", param->a2d_prof_stat.init_state);
        break;
    case ESP_A2D_SEP_REG_STATE_EVT:
        ESP_LOGI(TAG, "A2DP SEP registration state changed: %d", param->a2d_sep_reg_stat.reg_state);
        break;
    ...
    case ESP_A2D_SNK_SET_DELAY_VALUE_EVT:
        if (ESP_A2D_SET_INVALID_PARAMS == param->a2d_set_delay_value_stat.set_state) {
            ...
            ESP_LOGI(TAG, "Set delay report value: success, delay_value: %u * 1/10 ms",
                     param->a2d_set_delay_value_stat.delay_value);
        }
        break;
    case ESP_A2D_SNK_GET_DELAY_VALUE_EVT: {
        ESP_LOGI(TAG, "Get delay report value: delay_value: %u * 1/10 ms",
...
void SpeakerApp::handleGapEvent(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_ACL_CONN_CMPL_STAT_EVT:
        ...
        ESP_LOGI(TAG, "ESP_BT_GAP_ACL_CONN_CMPL_STAT_EVT Connected to [%02x:%02x:%02x:%02x:%02x:%02x], status: 0x%x",
                 bda[0], bda[1], bda[2], bda[3], bda[4], bda[5], param->acl_conn_cmpl_stat.stat);
        break;
    case ESP_BT_GAP_ACL_DISCONN_CMPL_STAT_EVT:
        bda = (uint8_t *)param->acl_disconn_cmpl_stat.bda;
        ESP_LOGI(TAG, "ESP_BT_GAP_ACL_DISC_CMPL_STAT_EVT Disconnected from [%02x:%02x:%02x:%02x:%02x:%02x], reason: 0x%x",
